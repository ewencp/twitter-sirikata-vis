...
var init = function() {
    ...
    system.self.onProxAdded(function(vis) {
        ...
        vis.onPositionChanged(set_position);
        set_position(vis);

        var set_size_color = function(vis) {
            var sc = vis.scale;
            if (sc > 1 && sc < 3) sc = 3; // to emphasize difference

            var col = sc-1;
            if (col > 16) col = 16;
            col = 127 + (8 * col);
            renderer.rectangle_set_color(rect_id, col, 127, 127);
            renderer.actor_set_size(rect_id, sc, sc);
        }
        vis.onScaleChanged(set_size_color);
        set_size_color(vis);
